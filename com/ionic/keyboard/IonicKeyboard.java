...
public class IonicKeyboard extends CordovaPlugin {
    ...
    class C00952 implements Runnable {
        ...
        C00952(CallbackContext callbackContext) {
    ...
    class C00963 implements Runnable {
        ...
        C00963(CallbackContext callbackContext) {
            this.val$callbackContext = callbackContext;
        }
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
        ...
        rootView.getViewTreeObserver().addOnGlobalLayoutListener(new C00941(rootView, density, appView));
    }

    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if ("close".equals(action)) {
            this.cordova.getThreadPool().execute(new C00952(callbackContext));
            return true;
        } else if (!"show".equals(action)) {
            return false;
        } else {
            this.cordova.getThreadPool().execute(new C00963(callbackContext));
            return true;
        }
    }
}
